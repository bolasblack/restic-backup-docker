...
if [ -n "${TEAMS_WEBHOOK_URL}" ]; then
    _TEAMS_WEBHOOK_LOGFILE="${lastLogfile}"
    if [ ! -z "$TEAMS_WEBHOOK_SIMPLE_SUMMARY" ]; then
        _TEAMS_WEBHOOK_LOGFILE="${lastResticLogfile}"
    fi

    /send_notification.py "${_TEAMS_WEBHOOK_LOGFILE}" "${lastMicrosoftTeamsLogfile}" "${TEAMS_WEBHOOK_URL}"
    if [ $? == 0 ]; then
